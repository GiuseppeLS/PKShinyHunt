...
export function App() {
  const { ready, initError, settings, setSettings, profiles, sessions, state, activeConfig, refreshSessions } = useAppState();
  const [tab, setTab] = useState("Dashboard");
  const [config, setConfig] = useState<HuntConfig>(activeConfig);

  useEffect(() => { setConfig(activeConfig); }, [activeConfig]);
  ...
  const run = async (fn: () => Promise<void>) => { await fn(); };

  return (
    <div className="layout">
      <Sidebar tab={tab} setTab={setTab} />
      <main>
        {initError && <div className="error-banner">âš  {initError}</div>}
        {tab === "Dashboard" && (
          <Dashboard
            state={state}
            onStart={() => run(async () => window.electronApi.startHunt(config))}
            onStop={() => run(async () => { await window.electronApi.stopHunt(); await refreshSessions(); })}
            onReset={() => run(async () => window.electronApi.resetSession())}
            onForceShiny={() => run(async () => { await window.electronApi.forceShiny(); await refreshSessions(); })}
        ...
        {tab === "Settings" && <SettingsPanel settings={settings} setSettings={setSettings} onSave={async () => { await window.electronApi.saveSettings(settings); }} />}
      </main>
    </div>
  );
}
